...
use objc2_metal::{
    ...
    MTLBuffer, MTLDevice, MTLLibrary, MTLPrimitiveType, MTLRenderPipelineState, MTLRenderStages,
    MTLResourceOptions,
};
...
pub use crate::kiln::swapchain::{RenderSurface, SwapchainConfig, PresentMode, ColorSpace};
...
impl Renderer {
    pub fn new<S: RenderSurface>(surface: &S, _swapchain: SwapchainConfig) -> Self {
        ...
        unsafe {
            ...
            lib_desc.setSource(Some(objc2_foundation::ns_string!(include_str!("./shaders/metal4_triangle.metal"))));
        ...
        unsafe { vfd.setName(Some(objc2_foundation::ns_string!("vertex_main"))); vfd.setLibrary(Some(&library)); }
        let ffd = unsafe { MTL4LibraryFunctionDescriptor::new() };
        unsafe { ffd.setName(Some(objc2_foundation::ns_string!("fragment_main"))); ffd.setLibrary(Some(&library)); }
        ...
        let vertex_buffer = unsafe { device.newBufferWithBytes_length_options(verts_ptr, verts_len, MTLResourceOptions::CPUCacheModeDefaultCache).expect("create vbuf") };

        Self { device, command_queue, command_allocator, pipeline_state, argument_table, scene_buffer, vertex_buffer }
    }
    ...
    pub fn draw_frame<S: RenderSurface>(&self, surface: &S, time: f32) {
        ...
        unsafe { core::ptr::copy_nonoverlapping(src_ptr, dst.as_ptr().cast::<u8>(), mem::size_of::<SceneProperties>()); }

        unsafe { self.argument_table.setAddress_atIndex(self.scene_buffer.gpuAddress(), 0); }
        unsafe { self.argument_table.setAddress_atIndex(self.vertex_buffer.gpuAddress(), 1); }

        let Some(rp) = surface.current_mtl4_render_pass_descriptor() else { return; };
        let Some(drawable) = surface.current_drawable() else { return; };

        unsafe { self.command_allocator.reset(); }
        let Some(cmd) = (unsafe { self.device.newCommandBuffer() }) else { return; };
        unsafe { cmd.beginCommandBufferWithAllocator(&self.command_allocator); }

        let Some(enc) = (unsafe { cmd.renderCommandEncoderWithDescriptor(&rp) }) else { return; };
        unsafe { enc.setRenderPipelineState(&self.pipeline_state); }
        unsafe { enc.setArgumentTable_atStages(&self.argument_table, MTLRenderStages::Vertex); }
        ...
        unsafe { self.command_queue.waitForDrawable(ProtocolObject::from_ref(&*drawable)); }
        let mut arr = [NonNull::from(&*cmd)];
        let ptr = unsafe { NonNull::new_unchecked(arr.as_mut_ptr()) };
        unsafe { self.command_queue.commit_count(ptr, 1); }
        unsafe { self.command_queue.signalDrawable(ProtocolObject::from_ref(&*drawable)); }
        drawable.present();
    }
}
...
#[allow(dead_code)]
pub fn now_time() -> f32 { NSDate::now().timeIntervalSinceNow() as f32 }
