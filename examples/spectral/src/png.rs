...
/// The shared workspace `target/` directory (honouring `CARGO_TARGET_DIR`), so headless renders
/// land alongside the test images regardless of crate. This crate's manifest is at
/// `<workspace>/examples/spectral`, so the workspace root is two levels up.
fn workspace_target_dir() -> std::path::PathBuf {
    ...
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("workspace root above examples/<crate>")
        .join("target")
}
