//! Spectral rendering data, split into the lighting and material
//! halves over a shared colorimetry core:
//!
//! The emission module contains SPDs, baked MIS sampling tables, and illuminant constructors;
//! reflectance contains the Peters moment-based representation; and the private data module holds
//! CIE/illuminant constant tables.
//!
...
//! is re-exported here, so callers use `spectrum::Spd`, `spectrum::fit_reflectance`,
...
pub enum Error {
    ...
    Invalid(&'static str),
    #[error("failed to read spectrum {path}: {source}")]
    Read {
