...
// Bring module tree into scope to mirror library paths used below.
use kiln::kiln as kiln;
            ...
            match event {
                ...
                | WindowEvent::Touch(_) => {
                    if let Some(mapped) = self.translator.process(&event) {
                        match mapped {
                            ...
                            kiln::events::AppEvent::MouseWheel { delta_x, delta_y, .. } =>
                                println!("wheel: {delta_x:.2},{delta_y:.2}"),
                            kiln::events::AppEvent::Key { state, key_code, repeat, text, .. } =>
                                println!("key: code={key_code} {:?} repeat={repeat} text={:?}", state, text),
                            kiln::events::AppEvent::Touch { id, phase, x, y, force } =>
                ...
                WindowEvent::Resized(size) => {
                    if let Some(surface) = self.surface.as_ref() {
                        unsafe {
                            surface.layer.setDrawableSize(CGSize { width: size.width as f64, height: size.height as f64 });
                        }
                    }
...
fn main() {
    ...
    use kiln::events::{AppEvent, ElementState, EventQueue, MouseButton};
    use objc2::rc::Retained;
    use objc2::runtime::ProtocolObject;
    ...
    // A minimal MTKView subclass that records AppKit input events
    ...
    define_class!(
        ...
        impl InputView {
            ...
            // Dragging with any button pressed
    ...
    // Add a normal Rust inherent method for shared logic (not an ObjC entry-point)
    impl InputView {
        fn handle_mouse_moved_rust(&self, event: &objc2_app_kit::NSEvent) {
            ...
            let flags = unsafe { event.modifierFlags() };
            let mut q = self.ivars().queue.borrow_mut();
            q.push(kiln::events::appkit_cursor_moved(loc.x as f64, loc.y as f64, flags));
        }
    }
    ...
    define_class!(
        ...
        unsafe impl NSApplicationDelegate for Delegate {
            ...
            unsafe fn applicationDidFinishLaunching(&self, _n: &NSNotification) {
                ...
                // Set a title for parity with the winit path and receive mouse moved events
                window.setTitle(ns_string!("kiln triangle (appkit)"));
                ...
                unsafe {
                    (&*view).as_super().setClearColor(MTLClearColor { red: 0.1, green: 0.1, blue: 0.12, alpha: 1.0 });
                ...
                unsafe {
                    lib_desc.setSource(Some(ns_string!(include_str!("../src/shaders/metal4_triangle.metal"))));
                    lib_desc.setName(Some(ns_string!("renderer_lib")));
                }
                let library: Retained<ProtocolObject<dyn MTLLibrary>> = unsafe { compiler.newLibraryWithDescriptor_error(&lib_desc).expect("create lib") };
                ...
                let scene_buffer = device
                    .newBufferWithLength_options(scene_buf_len, MTLResourceOptions::CPUCacheModeDefaultCache)
                ...
                let vertex_buffer = unsafe {
                    device
                        .newBufferWithBytes_length_options(verts_ptr, verts_len, MTLResourceOptions::CPUCacheModeDefaultCache)
                        .expect("create vbuf")
                };
                ...
                idcell_set!(command_allocator, self, unsafe { self.ivars().device.get().unwrap().newCommandAllocator().expect("create allocator") });
                ...
                // Make the view first responder and set the MTKView delegate to self so drawInMTKView is called
        ...
        unsafe impl MTKViewDelegate for Delegate {
            ...
            unsafe fn drawInMTKView(&self, view: &MTKView) {
                // Drain any queued AppKit events for parity with winit logging
            ...
            unsafe fn mtkView_drawableSizeWillChange(&self, _view: &MTKView, _size: NSSize) { /* no-op */ }
        }
    );
