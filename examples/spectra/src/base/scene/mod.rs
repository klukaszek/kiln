...
//!
//! Importers produce [`Scene`]. Renderers prepare a separate, backend-owned storage from it.
...
pub trait SceneStorage: Sized {
    ...
    fn build(device: &Device, source: &Scene, config: &Self::Config) -> Result<Self, Self::Error>;
