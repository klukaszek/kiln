...
impl<T: IntoBytes + FromBytes + Copy + Immutable> Generic<T> {
    ...
    fn write_all(&self, data: &[T]) -> Result<(), String> {
        if data.len() > self.len_elems {
            return Err("slice larger than buffer".into());
        }
        let bytes = data.as_bytes();
...
impl<T: IntoBytes + FromBytes + Copy + Immutable> Uniform<T> {
    pub fn from_slice(
        ...
        Self(Generic::from_slice(device, data, options))
    }
    pub fn with_len(
        device: &ProtocolObject<dyn MTLDevice>,
        len_elems: usize,
        ...
        Self(Generic::with_len(device, len_elems, options))
    }
    pub fn from_raw(buf: Retained<ProtocolObject<dyn MTLBuffer>>, len_elems: usize) -> Self {
        Self(Generic::from_raw(buf, len_elems))
    }
...
#[derive(Debug, Clone)]
pub struct IndexBuffer(IndexInner);
...
impl IndexBuffer {
    pub fn from_u16(
        ...
        data: &[u16],
        options: MTLResourceOptions,
    ) -> Self { Self(IndexInner::U16(Generic::from_slice(device, data, options))) }
    pub fn from_u32(
        device: &ProtocolObject<dyn MTLDevice>,
        data: &[u32],
        options: MTLResourceOptions,
    ) -> Self { Self(IndexInner::U32(Generic::from_slice(device, data, options))) }
    pub fn new_u16(device: &ProtocolObject<dyn MTLDevice>, data: &[u16], options: MTLResourceOptions) -> Self { Self::from_u16(device, data, options) }
    pub fn new_u32(device: &ProtocolObject<dyn MTLDevice>, data: &[u32], options: MTLResourceOptions) -> Self { Self::from_u32(device, data, options) }
    pub fn len(&self) -> usize { match &self.0 { IndexInner::U16(b) => b.len(), IndexInner::U32(b) => b.len() } }
    pub fn gpu_address(&self) -> u64 { match &self.0 { IndexInner::U16(b) => b.gpu_address(), IndexInner::U32(b) => b.gpu_address() } }
    pub fn index_type(&self) -> IndexType { match &self.0 { IndexInner::U16(_) => IndexType::U16, IndexInner::U32(_) => IndexType::U32 } }
}
...
// Free binding helpers removed; prefer ArgumentBuffer::bind2.
