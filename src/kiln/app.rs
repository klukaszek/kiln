...
use crate::kiln as kiln;

pub struct RunConfig<'a> { pub title: &'a str }
impl<'a> RunConfig<'a> { pub fn new(title: &'a str) -> Self { Self { title } } }
...
pub fn run(config: RunConfig, draw: DrawFn) {
    ...
    impl App { fn new(draw: DrawFn, title: &str) -> Self { Self { window: None, ns_view: None, surface: None, start: None, swapchain: kiln::swapchain::SwapchainConfig::default(), translator: kiln::events::WinitEventTranslator::new(), draw, title: title.to_string() } } }
    ...
    impl ApplicationHandler for App {
        ...
        fn window_event(&mut self, event_loop: &ActiveEventLoop, window_id: winit::window::WindowId, event: WindowEvent) {
            ...
            match event {
                WindowEvent::CloseRequested => { if let Some(view) = self.ns_view.take() { detach_layer_from_nsview(view); } request_app_exit(event_loop); }
                WindowEvent::ModifiersChanged(m) => { self.translator.update_modifiers(m); }
                WindowEvent::MouseInput { .. } | WindowEvent::CursorMoved { .. } | WindowEvent::MouseWheel { .. } | WindowEvent::KeyboardInput { .. } | WindowEvent::Touch(_) => {
                    if let Some(mapped) = self.translator.process(&event) {
                        match mapped {
                            kiln::events::AppEvent::MouseInput { button, state, .. } => println!("mouse click: {:?} {:?}", button, state),
                            kiln::events::AppEvent::CursorMoved { x, y, .. } => println!("cursor moved: {x:.1},{y:.1}"),
                            kiln::events::AppEvent::MouseWheel { delta_x, delta_y, .. } => println!("wheel: {delta_x:.2},{delta_y:.2}"),
                            kiln::events::AppEvent::Key { state, key_code, repeat, text, .. } => println!("key: code={key_code} {:?} repeat={repeat} text={:?}", state, text),
                            kiln::events::AppEvent::Touch { id, phase, x, y, force } => println!("touch: id={id} {:?} {x:.1},{y:.1} force={:?}", phase, force),
                            _ => {}
                        }
                    }
                }
                ...
                WindowEvent::RedrawRequested => {
                    if let (Some(surface), Some(start)) = (self.surface.as_ref(), self.start.as_ref()) {
        ...
        fn about_to_wait(&mut self, _event_loop: &ActiveEventLoop) { if let Some(window) = self.window.as_ref() { window.request_redraw(); } }
    }
...
#[cfg(all(feature = "winit", target_os = "macos"))]
pub fn run_app<A: ExampleApp + 'static>(mut example: A) {
    use core::cell::RefCell;
    use kiln::windowing::{apply_swapchain_to_metal_layer, request_app_exit};
    use objc2::rc::Retained;
    use objc2::runtime::ProtocolObject;
    ...
    #[derive(Debug)]
    struct Surface { layer: Retained<CAMetalLayer>, device: Retained<ProtocolObject<dyn MTLDevice>>, pending_drawable: RefCell<Option<Retained<ProtocolObject<dyn objc2_quartz_core::CAMetalDrawable>>>> }
    impl kiln::renderer::RenderSurface for Surface {
        fn current_mtl4_render_pass_descriptor(&self) -> Option<Retained<objc2_metal::MTL4RenderPassDescriptor>> {
            let Some(drawable) = (unsafe { self.layer.nextDrawable() }) else { return None; };
            self.pending_drawable.replace(Some(drawable.clone()));
            let rp = unsafe { objc2_metal::MTL4RenderPassDescriptor::new() };
            unsafe { let ca0 = rp.colorAttachments().objectAtIndexedSubscript(0); let tex = drawable.texture(); ca0.setTexture(Some(&tex)); }
            Some(rp)
        }
        fn current_drawable(&self) -> Option<Retained<ProtocolObject<dyn objc2_quartz_core::CAMetalDrawable>>> { self.pending_drawable.borrow_mut().take() }
        fn device(&self) -> Retained<ProtocolObject<dyn MTLDevice>> { self.device.clone() }
        fn color_pixel_format(&self) -> MTLPixelFormat { unsafe { self.layer.pixelFormat() } }
    }
    fn attach_layer_to_nsview(ns_view: *mut objc2::runtime::AnyObject, layer: &CAMetalLayer) { unsafe { let _: () = msg_send![ns_view, setWantsLayer: true]; } let ca_layer: &CALayer = (&*layer).as_super(); unsafe { let _: () = msg_send![ns_view, setLayer: Some(ca_layer)]; } }
    fn detach_layer_from_nsview(ns_view: *mut objc2::runtime::AnyObject) { unsafe { let _: () = msg_send![ns_view, setLayer: Option::<&CALayer>::None]; let _: () = msg_send![ns_view, setWantsLayer: false]; } }

    struct Handler { window: Option<Window>, ns_view: Option<*mut objc2::runtime::AnyObject>, surface: Option<Surface>, start: Option<Instant>, last: Option<Instant>, swapchain: kiln::swapchain::SwapchainConfig, app: Box<dyn ExampleApp> }
    impl Handler { fn new(app: Box<dyn ExampleApp>) -> Self { Self { window: None, ns_view: None, surface: None, start: None, last: None, swapchain: kiln::swapchain::SwapchainConfig::default(), app } } }
    impl ApplicationHandler for Handler {
        fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            if self.window.is_some() { return; }
            let attrs = WindowAttributes::default().with_title(self.app.title());
            let window = event_loop.create_window(attrs).expect("create window");
            let ns_view: *mut objc2::runtime::AnyObject = { let wh = window.window_handle().unwrap(); let raw = wh.as_raw(); match raw { RawWindowHandle::AppKit(h) => h.ns_view.as_ptr().cast(), _ => core::ptr::null_mut() } };
            assert!(!ns_view.is_null(), "Expected AppKit NSView");
            let device = MTLCreateSystemDefaultDevice().expect("no system device");
            let layer = unsafe { CAMetalLayer::layer() };
            let sc = self.swapchain;
            unsafe { layer.setDevice(Some(&device)); let size = window.inner_size(); apply_swapchain_to_metal_layer(&layer, size.width as f64, size.height as f64, &sc); }
            attach_layer_to_nsview(ns_view, &layer);
            let surface = Surface { layer: layer.clone(), device: device.clone(), pending_drawable: RefCell::new(None) };
        ...
        fn window_event(&mut self, event_loop: &ActiveEventLoop, window_id: winit::window::WindowId, event: WindowEvent) {
            ...
            if window.id() != window_id { return; }
            match event {
                WindowEvent::CloseRequested => { self.app.quit(); if let Some(view) = self.ns_view.take() { detach_layer_from_nsview(view); } request_app_exit(event_loop); }
                WindowEvent::Resized(size) => { if let Some(surface) = self.surface.as_ref() { unsafe { surface.layer.setDrawableSize(CGSize { width: size.width as f64, height: size.height as f64 }); } } window.request_redraw(); }
                WindowEvent::RedrawRequested => {
                    if let (Some(surface), Some(start)) = (self.surface.as_ref(), self.start.as_ref()) {
                        let now = Instant::now(); let dt = self.last.map(|lf| now.duration_since(lf).as_secs_f32()).unwrap_or(0.0); self.last = Some(now);
                        self.app.update(dt);
                        let t = -start.elapsed().as_secs_f32(); self.app.draw(surface, t);
                    }
                }
                _ => {}
            }
...
pub fn run(config: RunConfig, draw: DrawFn) {
    ...
    use objc2_foundation::{ NSString, NSDate, NSNotification, NSObject, NSObjectProtocol, NSPoint, NSRect, NSSize };
    ...
    impl InputView { fn handle_mouse_moved_rust(&self, event: &objc2_app_kit::NSEvent) { let loc_win = unsafe { event.locationInWindow() }; let loc = (&*self).as_super().convertPoint_fromView(loc_win, None); let flags = unsafe { event.modifierFlags() }; let mut q = self.ivars().queue.borrow_mut(); q.push(kiln::events::appkit_cursor_moved(loc.x as f64, loc.y as f64, flags)); } }
    ...
    define_class!(
        ...
        unsafe impl NSApplicationDelegate for Delegate {
            ...
            unsafe fn applicationDidFinishLaunching(&self, _n: &NSNotification) {
                ...
                if let Some(t) = self.ivars().title.get() { window.setTitle(t); }
                window.setAcceptsMouseMovedEvents(true);

                let device = objc2_metal::MTLCreateSystemDefaultDevice().expect("failed to get default system device");
                ...
                unsafe { (&*view).as_super().setPaused(false); (&*view).as_super().setPreferredFramesPerSecond(60); }
                ...
                unsafe { (&*v).as_super().setDelegate(Some(del)); }
            ...
            unsafe fn drawInMTKView(&self, view: &MTKView) {
                if let Some(v) = self.ivars().view.get() {
                    v.ivars().queue.borrow_mut().drain(|ev| {
                        match ev {
                            AppEvent::MouseInput { button, state, .. } => println!("mouse click: {:?} {:?}", button, state),
                            AppEvent::CursorMoved { x, y, .. } => println!("cursor moved: {x:.1},{y:.1}"),
                            AppEvent::MouseWheel { delta_x, delta_y, .. } => println!("wheel: {delta_x:.2},{delta_y:.2}"),
                            AppEvent::Key { state, key_code, repeat, text, .. } => println!("key: code={key_code} {:?} repeat={repeat} text={:?}", state, text),
                            AppEvent::Touch { id, phase, x, y, force } => println!("touch: id={id} {:?} {x:.1},{y:.1} force={:?}", phase, force),
                            AppEvent::RedrawRequested | AppEvent::CloseRequested => {}
                        }
                    });
                }
                if let Some(draw_cell) = self.ivars().draw.get() {
                    struct ViewSurface<'a> { v: &'a MTKView, d: Retained<ProtocolObject<dyn objc2_metal::MTLDevice>> }
                    impl<'a> kiln::swapchain::RenderSurface for ViewSurface<'a> {
                        fn current_mtl4_render_pass_descriptor(&self) -> Option<Retained<objc2_metal::MTL4RenderPassDescriptor>> { unsafe { self.v.currentMTL4RenderPassDescriptor() } }
                        fn current_drawable(&self) -> Option<Retained<ProtocolObject<dyn objc2_quartz_core::CAMetalDrawable>>> { unsafe { self.v.currentDrawable() } }
                        fn device(&self) -> Retained<ProtocolObject<dyn objc2_metal::MTLDevice>> { self.d.clone() }
                        fn color_pixel_format(&self) -> objc2_metal::MTLPixelFormat { unsafe { self.v.colorPixelFormat() } }
                    }
                    let device = self.ivars().device.get().unwrap();
                    let surface = ViewSurface { v: view, d: device.clone() };
                    (draw_cell.borrow_mut())(&surface, self.ivars().start_date.timeIntervalSinceNow() as f32);
                }
            }
    ...
    let delegate: Retained<Delegate> = {
        ...
        let this = this.set_ivars(Ivars { start_date: NSDate::now(), device: OnceCell::default(), window: OnceCell::default(), view: OnceCell::default(), title: OnceCell::default(), draw: OnceCell::default() });
