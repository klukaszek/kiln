...
// ---------------------------------------------------------------------------
// Ray tracing pipeline — Aaltonen-philosophy RT pipeline
// ---------------------------------------------------------------------------

/// A single shader group within a ray tracing PSO.
///
/// Following the Aaltonen model: shader groups are identified by index.
/// The shader binding table (`SbtRegion`) is built by the caller using the group
/// indices returned at PSO creation time.
#[derive(Clone, Debug)]
pub enum RayTracingShaderGroup {
    /// Ray generation shader (one per pipeline).
    RayGen { shader: usize },
    /// Miss shader invoked when no geometry is intersected.
    Miss { shader: usize },
    /// Triangle hit group: closest-hit and optional any-hit.
    TriangleHit {
        closest_hit: usize,
        any_hit: Option<usize>,
    },
    /// Procedural hit group: intersection + optional closest-hit + optional any-hit.
    ProceduralHit {
        intersection: usize,
        closest_hit: Option<usize>,
        any_hit: Option<usize>,
    },
    /// General callable shader.
    Callable { shader: usize },
}

/// Description for creating a ray tracing pipeline.
///
/// Aaltonen philosophy: no descriptor sets, no binding tables in the API —
/// the caller writes the SBT into GPU memory directly and passes the regions
/// as `GpuAddress` + stride + size to `cmd.trace_rays(...)`.
#[derive(Clone, Debug)]
pub struct RayTracingPsoDesc {
    /// All shader modules referenced by `groups`.
    /// The same module may be referenced by multiple groups.
    pub shaders: Vec<usize>,
    /// Shader groups that define the pipeline.
    pub groups: Vec<RayTracingShaderGroup>,
    /// Maximum recursion depth (1 = no secondary rays from hit shaders).
    pub max_recursion_depth: u32,
    /// Maximum size in bytes of a ray payload struct.
    pub max_payload_size: u32,
    /// Maximum size in bytes of a hit attribute struct (e.g. barycentric coords = 8 bytes).
    pub max_attribute_size: u32,
    pub label: Option<String>,
}

/// Opaque ray tracing pipeline handle.
pub struct RayTracingPso {
    pub(crate) inner: RayTracingPsoInner,
}

pub(crate) enum RayTracingPsoInner {
    #[cfg(feature = "vulkan")]
    Vulkan(Box<crate::backend::vulkan::pipeline::VulkanRayTracingPso>),
    #[cfg(feature = "metal")]
    Metal(Box<crate::backend::metal::pipeline::MetalRayTracingPso>),
}
