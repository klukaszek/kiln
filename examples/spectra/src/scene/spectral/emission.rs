...
use super::{LAMBDA_MAX, LAMBDA_MIN, interp};
#[cfg(test)]
...
impl Spd {
    ...
    fn from_samples(name: String, mut samples: Vec<(f32, f32)>) -> anyhow::Result<Self> {
        anyhow::ensure!(samples.len() >= 2, "an SPD needs at least two samples");
        samples.sort_by(|a, b| a.0.total_cmp(&b.0));
        anyhow::ensure!(
            samples
                .iter()
                .all(|&(nm, power)| nm.is_finite() && power.is_finite() && power >= 0.0),
            "SPD contains non-finite wavelengths or invalid power values"
        );
        anyhow::ensure!(
            samples.windows(2).all(|pair| pair[0].0 < pair[1].0),
            "SPD contains duplicate wavelengths"
        );
...
pub fn from_lspdd_csv(path: &std::path::Path) -> anyhow::Result<Spd> {
    ...
    Spd::from_samples(name, samples).with_context(|| format!("invalid SPD in {}", path.display()))
}
