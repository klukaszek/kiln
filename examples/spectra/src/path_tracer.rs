...
use crate::render::{self, Error, FrameArenas, PresentRenderer, RenderFrame, Renderer};
...
impl PathTracer {
    pub fn new(
        ...
        let trace_scene = TraceScene::build(device, scene, light_spectrum)?;
        let pipelines = match Pipelines::new(
            device,
            color_format,
            schedule.pixel_stride,
            trace_scene.lights.len(),
            trace_scene.spectrum.len(),
        ) {
